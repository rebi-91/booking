...
function StaffLog() {
  ...
  const [tempTimeDisplay, setTempTimeDisplay] = useState<{ [key: string]: string }>({});
  const [showSummaryModal, setShowSummaryModal] = useState(false);
  const [summaryStaff, setSummaryStaff] = useState<any | null>(null);
  const [summaryEntries, setSummaryEntries] = useState<string[]>([]);
  ...
  const openActionModal = (staff: any, action: "in" | "startBreak" | "endBreak" | "out") => {
    ...
    if (action === "out") {
      ...
      if (startBreak && !endBreak) return alert("Cannot logout before ending break.");
    }
  ...
  const confirmAction = () => {
    ...
    performUpdate(); // Proceed to try to update
  };
  const performUpdate = async () => {
    if (!modalStaff || !modalAction) return;
  
    const field = `${modalAction}${D}`;
    if (modalStaff[field]) return; // Skip update if already entered
    ...
    try {
      ...
      setStaffList((list) =>
        list.map((s) =>
          s.staffID === modalStaff.staffID ? updatedStaff : s
        )
      );
      ...
      // Store temp time for 3 seconds
      setTempTimeDisplay((prev) => ({
        ...prev,
        [`${modalStaff.staffID}_${field}`]: now,
      }));
      setTimeout(() => {
        setTempTimeDisplay((prev) => {
          const copy = { ...prev };
          delete copy[`${modalStaff.staffID}_${field}`];
          return copy;
        });
      }, 3000);
  
      // ✅ Build the summary entries
      const actions = ["in", "startBreak", "endBreak", "out"];
      ...
      const entries = actions
        .map((action) => {
          const time = updatedStaff[`${action}${D}`];
          return time ? `${labels[action]}: ${time}` : null;
        })
        .filter(Boolean) as string[];
  
      setSummaryStaff(updatedStaff);
  ...
  const totalHours = selectedStaff
    ? daysArray.reduce((sum, d) => {
        ...
        if (inT && outT) {
          ...
          const worked = (oh * 60 + om) - (ih * 60 + im) - ((eh * 60 + em) - (sh * 60 + sm));
            ...
            <div className="top-table-inner">
              <Table bordered hover className="table-custom text-center">
                ...
                <tbody>
                  {staffList.map(st => (
                    <tr key={st.staffID}>
                      <td className="fw-bold clickable" onClick={() => handleNameClick(st)}>{st.staffName}</td>
                      <td>
                        <Button size="sm" onClick={() => openActionModal(st, "in")}>Login</Button>
                      </td>
                      <td>
                        <Button size="sm" onClick={() => openActionModal(st, "startBreak")}>Start Break</Button>
                      </td>
                      <td>
                        <Button size="sm" className="btn-endbreak" onClick={() => openActionModal(st, "endBreak")}>End Break</Button>
                      </td>
                      <td>
                        <Button size="sm" className="btn-logout" onClick={() => openActionModal(st, "out")}>Logout</Button>
                      </td>
                      <td className="fw-bold clickable" onClick={() => handleNameClick(st)}>{st.staffName}</td>
                    </tr>
                  ))}
          ...
          <Col xs={12} md={10} lg={8}>
            <div className="bg-dark p-4 rounded shadow mb-4">
              ...
              <div className="pie-button-wrapper d-flex justify-content-center align-items-center">
                ...
                <Button variant="outline-light" className="ms-4 rounded show-attendance-btn" onClick={() => setShowAttendanceTable(!showAttendanceTable)}>
...
<Modal show={showSummaryModal} onHide={() => setShowSummaryModal(false)} centered>
  <Modal.Header closeButton>
    <Modal.Title>Time Entry for {summaryStaff?.staffName}</Modal.Title>
  </Modal.Header>
  <Modal.Body>
    {summaryEntries.map((entry, idx) => (
      <div key={idx}><strong>{entry}</strong></div>
    ))}
  </Modal.Body>
  <Modal.Footer>
    <Button variant="primary" onClick={() => setShowSummaryModal(false)}>OK</Button>
  </Modal.Footer>
</Modal>
...
//   const openActionModal = (staff: any, action: "in" | "startBreak" | "endBreak" | "out") => {
//     const field = `${action}${D}`;
//     const existingRecorded = staff[field];
//     const tempShown = tempTimeDisplay[`${staff.staffID}_${field}`];

//     if (existingRecorded && !tempShown) {
//       setIdModalStaff(staff);
//       setIdInput("");
//       setShowIDModal(true);
//       return;
...
//     setUpdateLoading(true);
//     try {
//       await supabase.from("attendance").update({ [field]: now }).eq("staffID", modalStaff.staffID);
...
//                     const lo = st[`out${D}`];
//                     const keyBase = `${st.staffID}_${D}`;
//                     return (
//                       <tr key={st.staffID}>
//                         <td className="fw-bold clickable" onClick={() => handleNameClick(st)}>{st.staffName}</td>
//                         <td>
//                           {tempTimeDisplay[`${st.staffID}_in${D}`] ? (
...
//                         </td>
//                         <td>
//                           {tempTimeDisplay[`${st.staffID}_startBreak${D}`] ? (
//                             <span style={{ fontWeight: "bold", color: "#28a745" }}>
//                               {tempTimeDisplay[`${st.staffID}_startBreak${D}`]}
//                             </span>
//                           ) : sb ? (
...
//                         </td>
//                         <td>
//                           {tempTimeDisplay[`${st.staffID}_endBreak${D}`] ? (
//                             <span style={{ fontWeight: "bold", color: "#28a745" }}>
//                               {tempTimeDisplay[`${st.staffID}_endBreak${D}`]}
//                             </span>
//                           ) : eb ? (
...
//                         </td>
//                         <td>
//                           {tempTimeDisplay[`${st.staffID}_out${D}`] ? (
//                             <span style={{ fontWeight: "bold", color: "#28a745" }}>
//                               {tempTimeDisplay[`${st.staffID}_out${D}`]}
//                             </span>
//                           ) : lo ? (
...
//     // If already recorded and temporary display expired: open ID modal only
//     if (existingRecorded && !tempShown) {
//       setIdModalStaff(staff);
//       setIdInput("");
//       setShowIDModal(true);
...
//       // Show the time temporarily in table
//       setTempTimeDisplay(prev => ({
//         ...prev,
//         [`${modalStaff.staffID}_${field}`]: now,
//       }));
...
//     const field = `${modalAction}${D}`;
//     const existingTime = modalStaff[field];

//     if (existingTime) {
//       alert(`Time already recorded: ${existingTime}`);
//       setShowActionModal(false);
//       setInpStaffID(""); 
//       setTimeout(() => setShowActionModal(true), 100);
//       return;
//     }
...
//       setTempTimeDisplay(prev => ({ ...prev, [`${modalStaff.staffID}_${field}`]: now }));
//       setTimeout(() => {
//         setTempTimeDisplay(prev => {
//           const newDisplay = { ...prev };
//           delete newDisplay[`${modalStaff.staffID}_${field}`];
//           return newDisplay;
//         });
//       }, 3000);
//     } catch {
...
//                           </td>
//                           <td>
//                             {tempTimeDisplay[`${st.staffID}_in${D}`] || li || (
...
//                           </td>
//                           <td>
//                             {tempTimeDisplay[`${st.staffID}_startBreak${D}`] || sb || (
...
//                           </td>
//                           <td>
//                             {tempTimeDisplay[`${st.staffID}_endBreak${D}`] || eb || (
...
//                           </td>
//                           <td>
//                             {tempTimeDisplay[`${st.staffID}_out${D}`] || lo || (
