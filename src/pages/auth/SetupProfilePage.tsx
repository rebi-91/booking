...
// File: src/pages/SetupProfilePage.tsx
...
export default function SetupProfilePage() {
  ...
  const [dobValue, setDobValue] = useState(""); // YYYY-MM-DD
  const [countryCode, setCountryCode] = useState("44"); // default +44
  const [phone, setPhone] = useState(""); // may type leading zero
  const [email, setEmail] = useState("");
  ...
  // disable Next if required empty
  const isNextDisabled = (() => {
    switch (currentStep) {
      case 1:
        return false;
      case 2:
        if (role === "Staff") return password.trim() === "";
        return (
          title === "" ||
          name.trim() === "" ||
          dobValue.trim() === "" ||
      ...
      >
        ...
        {currentStep < TOTAL_STEPS ? (
          ...
          <div className="sp-confirm">
            ...
            <button className="sp-confirm-btn" onClick={handleConfirm}>
              Confirm &amp; Finish
            </button>
          </div>
        )}
