...
function DashBoard() {
  ...
  /**
   * When a checkbox cell is hovered, use the provided group info (from the header)
   * if no filter is selected.
   * @param e - mouse event from the cell
   * @param classTime - the column (e.g., "C1")
   * @param group - the group header text ("ClassName - Section Section")
   */
  const handleCheckboxHover = async (e: React.MouseEvent, classTime: string, group?: string) => {
    if (!userSchool) return;
    // Determine which className/section to use:
    let classNameToUse = selectedClassName;
    let sectionToUse = selectedSection;
    if (!selectedClassName || !selectedSection) {
      if (group) {
        // group is in the format "ClassName - Section Section"
                ...
                <tr>
                  ...
                  {classTimes.map((classTime) => (
                    <th
                      ...
                      style={styles.th}
                      onMouseEnter={(e) => 
                        // Pass the current group header text as third parameter.
                        // (If filters are not set, this will be used.)
                        handleCheckboxHover(e, classTime, "")
                      }
                      onMouseLeave={handleCheckboxLeave}
                    >
                      {classTime}
                        ...
                        <tr key={student.id} style={styles.tr}>
                          ...
                          {classTimes.map((classTime) => (
                            <td
                              key={classTime}
                              style={styles.td}
                              onMouseEnter={(e) => handleCheckboxHover(e, classTime, group)}
                              onMouseLeave={handleCheckboxLeave}
                            >
                              {attendanceData[student.id] && attendanceData[student.id][classTime] === true ? (
                                <label style={styles.checkboxLabel}>
