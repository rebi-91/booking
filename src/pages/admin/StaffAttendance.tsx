...
//       fetchTeachers(profileData.school, selectedDate);
//     } catch (error: any) {
...
//   // -----------------------------------
//   // Count how many columns are TRUE (1..31 vs e1..e31)
//   // -----------------------------------
...
//       // Re-fetch
//       fetchTeachers(userSchool, selectedDate);
...
//                   {/* Start (renamed from Morning) */}
//                   <td style={styles.td}>
//                     {getAttendanceDisplay(teacher.Start)}
//                   </td>

//                   {/* Finish (renamed from Evening) */}
//                   <td style={styles.td}>
//                     {getAttendanceDisplay(teacher.Finish)}
//                   </td>
...
//                   {/* Login (Days) => teacher.present */}
//                   <td style={styles.td}>{teacher.present}</td>

//                   {/* Logout (Days) => teacher.presentEvening */}
...
//       {/* Add Staff Modal */}
//       <Modal
//         show={addStaffModalVisible}
//         onHide={() => setAddStaffModalVisible(false)}
//         centered
//       >
...
//           <Button
//             variant="secondary"
//             onClick={() => setAddStaffModalVisible(false)}
...
function StaffAttendance() {
    ...
    try {
      ...
      for (const teacher of data as any[]) {
        ...
        if (updateNeeded) {
          ...
          if (updateError) {
            console.error(`Error updating present counts for Teacher ID ${teacher.teacherID}:`, updateError.message);
          }
        }
  ...
  const handleAddStaff = async () => {
    ...
    try {
      ...
      // Optionally, initialize other minLateN columns to NULL or default values
      // Uncomment the following lines if you want to initialize all minLateN columns
      /*
      for (let i = 1; i <= 31; i++) {
        const col = `minLate${i}`;
        if (!insertData.hasOwnProperty(col)) {
          insertData[col] = null; // or 0 based on your requirements
        }
      }
      */
...
//   minLate: number | null;          // Fetched dynamically from teacher["minLateN"]
//   totalLate: number | null;
//   presentMorning: number | null;   // Mapped to teacher.present
//   presentEvening: number | null;   // Mapped to teacher.presentEvening
//   days: Record<string, boolean | number | null>;
// }
...
//       const day = date.getDate();
//       const dayStr = day.toString();
//       const eveningDayStr = `e${day}`;      // e.g. 'e3'
//       const loginColumn = `in${day}`;       // e.g. 'in3'
//       const logoutColumn = `out${day}`;     // e.g. 'out3'
//       const minLateColumn = `minLate${day}`; // e.g. 'minLate3'
...
//       // Map data to our AttendanceRow format
...
//         return {
//           index: teacher.id,
//           // Dynamically get login/logout times
//           Login: teacher[loginColumn] || null,
//           Logout: teacher[logoutColumn] || null,
...
//           // Dynamically get Start/Finish from dayStr, eveningDayStr
//           Start: teacher[dayStr] !== undefined ? teacher[dayStr] : null,
//           Finish: teacher[eveningDayStr] !== undefined ? teacher[eveningDayStr] : null,

//           // Dynamically fetch today's minLate
//           minLate: teacher[minLateColumn] || 0,
//           totalLate: teacher.totalLate || 0,

//           // Keep existing present columns for total days absent or present
//           presentMorning: teacher.present || 0,
//           presentEvening: teacher.presentEvening || 0,

//           days,
//         };
//       });

//       // Optionally, recalc any totals if needed
//       // We'll assume 'present' columns store the count of how many times they've been absent, 
//       // or present, depending on your logic. Example below:
//       const todayNum = date.getDate();
//       for (let tchr of formattedData) {
//         const daysAbsentStart = calculateDaysAbsent(tchr.days, todayNum, false);
//         const daysAbsentFinish = calculateDaysAbsent(tchr.days, todayNum, true);
//         let updateNeeded = false;
//         const updateData: any = {};

//         // Example: if presentMorning = how many times they've been absent in the morning
//         if (tchr.presentMorning !== daysAbsentStart) {
//           updateData.present = daysAbsentStart;
//           tchr.presentMorning = daysAbsentStart;
//           updateNeeded = true;
//         }

//         // If presentEvening = how many times they've been absent in the evening
//         if (tchr.presentEvening !== daysAbsentFinish) {
//           updateData.presentEvening = daysAbsentFinish;
//           tchr.presentEvening = daysAbsentFinish;
//           updateNeeded = true;
//         }
...
//   // -----------------------------------
//   // Calculate Days Absent up to the selected date
//   // If isEvening = false => we look at day i
//   // If isEvening = true  => we look at day e{i}
//   // -----------------------------------
...
//   // -----------------------------------
//   // useEffect to filter teachers array based on searchTerm and new filters
//   // -----------------------------------
//   useEffect(() => {
//     let temp = [...teachers];
...
//     // Filter NOT Present in Start
//     if (filterNotPresentStart) {
//       temp = temp.filter((t) => t.Start === false || t.Start === null);
//     }
//     // Filter Present in Start
//     if (filterPresentStart) {
//       temp = temp.filter((t) => t.Start === true);
//     }

//     // Filter NOT Present in Finish
//     if (filterNotPresentFinish) {
//       temp = temp.filter((t) => t.Finish === false || t.Finish === null);
//     }
//     // Filter Present in Finish
//     if (filterPresentFinish) {
//       temp = temp.filter((t) => t.Finish === true);
//     }
...
//   // -----------------------------------
//   // Long Press to edit fields (telNumber, teacherName)
//   // -----------------------------------
//   const handleLongPressStart = (
//     e: React.MouseEvent | React.TouchEvent,
//     teacherID: string,
...
//   // -----------------------------------
//   // On blur => Save changes to supabase
//   // -----------------------------------
//   const handleFieldBlur = async (teacherID: string, field: string) => {
//     const newValue = fieldValues[field as keyof typeof fieldValues].trim();
//     try {
...
//       // Re-fetch teachers to see the updated field
//       fetchTeachers(userSchool, selectedDate);
...
//   // -----------------------------------
//   // Render Start or Finish as a checkbox if true, or ❌ if false/null, 
//   // or number in red if it's a numeric value
//   // -----------------------------------
//   const getAttendanceDisplay = (value: boolean | number | null) => {
//     if (value === true) {
//       return (
...
//             {/* <-- Create a bit more gap between second and third filters --> */}
//             <div style={{ marginBottom: "10px" }} />
...
//                 <th style={styles.th}>mins Late</th>
//                 <th style={styles.th}>Total mins Late</th>
//                 {/* "Morning (Days)" => "Login (Days)" => from teacher.presentMorning */}
//                 <th style={styles.th}>Login (Days)</th>
//                 {/* "Evening (Days)" => "Logout (Days)" => from teacher.presentEvening */}
//                 <th style={styles.th}>Logout (Days)</th>
//               </tr>
//             </thead>
//             <tbody>
...
//                   {/* Mins Late (from minLateN) */}
//                   <td style={styles.td}>{teacher.minLate || 0}</td>

//                   {/* Total mins Late */}
//                   <td style={styles.td}>{teacher.totalLate || 0}</td>

//                   {/* "Login (Days)" => from teacher.presentMorning */}
//                   <td style={styles.td}>{teacher.presentMorning}</td>

//                   {/* "Logout (Days)" => from teacher.presentEvening */}
//                   <td style={styles.td}>{teacher.presentEvening}</td>
//                 </tr>
//               ))}
//             </tbody>
