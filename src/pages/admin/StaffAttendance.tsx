...
import { Modal, Button, Spinner, Form, Container, Row, Col, Table, InputGroup, FormControl } from "react-bootstrap";
...
interface AttendanceRow {
  ...
  Start: boolean | number | null;
  Finish: boolean | number | null;
  minLate: number | null;
  totalLate: number | null;
  present: number | null;
  presentEvening: number | null;
  days: Record<string, boolean | number | null>;
}

// Hook to detect mobile screen (adjust breakpoint as needed)
const useIsMobile = () => {
...
function StaffAttendance() {
    ...
    try {
      ...
      for (const teacher of data as any[]) {
        ...
        if (updateNeeded) {
          ...
          if (updateError) {
            console.error(`Error updating counts for ${teacher.teacherID}:`, updateError.message);
          }
        }
        newRows.push(row);
      }
  ...
  const countTrueValues = (days: Record<string, boolean | number | null>, isEvening: boolean): number => {
    let count = 0;
    for (let i = 1; i <= 31; i++) {
      const key = isEvening ? `e${i}` : i.toString();
      if (days[key] === true) count++;
    }
    return count;
  };
  ...
  }, [searchTerm, teachers, filterNotPresentStart, filterPresentStart, filterNotPresentFinish, filterPresentFinish]);
  ...
  const handleSelectAllChange = () => {
    if (selectAll) {
      ...
      const allIDs = filteredTeachers.map((t) => t.teacherID!);
  ...
  // -----------------------------------
  // Delete staff (with password confirmation)
  // -----------------------------------
  const handleDeleteStaff = async () => {
    if (deletePassword !== "342122") {
  ...
  const getAttendanceDisplay = (value: boolean | number | null) => {
    if (value === true) {
      ...
      return <span className="fw-bold text-danger">{value}</span>;
    } else {
      return "---";
    }
  };
  ...
  return (
    <Container fluid className="staff-attendance-container">
      <Row className="justify-content-center">
        <Col xs={12} md={10} lg={8} className="text-center my-3">
      ...
      <div className="floating-nav">
        {userRole === "ADMIN" && (
          <Button variant="success" className="nav-btn" onClick={() => navigate("/dashboard3")}>
            +
          </Button>
        )}
        <Button variant="primary" className="nav-btn" onClick={() => navigate("/")}>
          🏠
        </Button>
        <Button variant="info" className="nav-btn" onClick={() => navigate("/dashboard")}>
          📅
        </Button>
        {userRole === "ACCOUNTANT" && (
          <Button variant="dark" className="nav-btn" onClick={() => navigate("/dashboard4")}>
            💵
          </Button>
        )}
      </div>
      ...
      <Container className="mb-4">
        <Row>
          ...
          {/* Add Staff & DatePicker */}
          <Col xs={12} md={6} className="mb-3">
            <div className="add-staff-card p-3 text-center">
              <h2 className="h4">Add New Staff</h2>
              <Button variant="primary" onClick={() => setAddStaffModalVisible(true)} className="mb-3">
                ➕ Add Staff
              </Button>
              <div className="date-picker-wrapper">
                <h4 className="h6 text-white">Select Day</h4>
                <DatePicker
      ...
      {selectedTeachers.size > 0 && (
        <Container className="mb-3">
          <Button variant="danger" onClick={() => setDeleteConfirmVisible(true)}>
            Delete Staff
          </Button>
        </Container>
      )}

      <Container fluid className="table-responsive mb-5">
        {isLoading ? (
            ...
            <tbody>
              {filteredTeachers.map((teacher) => (
                <tr key={teacher.teacherID}>
                  ...
                  <td>{getAttendanceDisplay(teacher.Start)}</td>
                  <td>{getAttendanceDisplay(teacher.Finish)}</td>
                  <td>{teacher.minLate || 0}</td>
                  <td>{teacher.totalLate || 0}</td>
                  <td>{teacher.present}</td>
                  <td>{teacher.presentEvening}</td>
                </tr>
              ))}
            </tbody>
          </Table>
        ) : (
          <p className="text-center text-light">No staff found for the selected filters.</p>
        )}
      </Container>
      ...
      <Modal show={addStaffModalVisible} onHide={() => setAddStaffModalVisible(false)} centered>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setAddStaffModalVisible(false)}>
            Cancel
          </Button>
      ...
      <Modal show={deleteConfirmVisible} onHide={() => { setDeleteConfirmVisible(false); setDeletePassword(""); }} centered>
        <Modal.Header closeButton>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => { setDeleteConfirmVisible(false); setDeletePassword(""); }}>
            Cancel
          </Button>
